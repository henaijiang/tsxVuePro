    ...
    return (
      <div class="class1" style={{ "--primarycolor": "red" }}>
        <Rhomboid
          textLabel="测试"
          boxColor="red"
          on-handleClick={vm.handleClick}
        />
        <br />
        <Button type="primary" ref="button" onClick={vm.clickMethod}>
          点我换肤
        </Button>
        <br />
        <el-dialog
          width="30%"
          visible={vm.themeVisible}
          {...{
            on: {
              "update:visible": (visible: boolean) => {
                vm.themeVisible = visible;
              }
            }
          }}
        >
          <el-collapse>
            <ElTheme
              on-themeConfigChange={vm.themeConfigChange}
              themeConfig={vm.themeConfig}
            />
          </el-collapse>
        </el-dialog>
        {vm.num ? (
        ...
        <TestChild
          ...
          scopedSlots={{
            default: (props: any) => {
              return <div style="line-height: 60px;">{props}</div>;
            }
          }}
        >
          <p style={{ color: "red" }} slot="header">
            我是插槽header内容
          </p>
          <p style={{ color: "blue" }} slot="main">
            我是插槽main内容
          </p>
          <p style={{ color: "black" }} slot="footer">
            我是插槽footer内容
          </p>
        </TestChild>
